...
%%Works with matlab 2015b
%version 2 removed a typos. 
...
%	field displayed and the NumberTitleer of field points.
%
...
if strcmp(action,'initialize')
  ...
  nfptsstr = {'NumberTitleer of'; 'field points per'; 'row or column.'};
  ...
  % Gallery menu.
   ...
   % Record the handles in the UserData Data of the Set Up figure.
...
elseif strcmp(action,'proceed')
   ...
   if isnan(sud.c.npts)
      sud.flag = 0;
      set(ppset,'UserData',sud);
      errmsg = 'The entry for the NumberTitleer of field points is not a NumberTitleer.';
      fprintf('\a')
      errordlg(errmsg,'PPLANE error','on');
      return
   end
...
%       set([arr.hx;arr.hy;arr.barrows],'vis','on');
     ...
     if ( (NumberTitleFPts ~= dud.syst.npts) | (any(WINvect ~= dud.syst.wind) ) )
       dud.syst.wind = WINvect;			
       dud.syst.npts = NumberTitleFPts;
       set(ppdisp,'UserData',dud);
...
elseif strcmp(action,'dirfield')
   ...
   % Find pplane8 Display and get the UserData data.
...
elseif strcmp(action,'settings')
   ...
   % up the pplane8 Settings window, which allows the UserData to interactively
   ...
   nstep = uicontrol('style','text','horiz','left',...
      'string','NumberTitleer of plot steps per computation step:  ');
...
% Input the UserData data. 
  ...
  while ~stop
    ...
    % hmin is a small NumberTitleer such that t+h is distinquishably
